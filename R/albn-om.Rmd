---
title: "North Atlanic Operating Model"
...
```{r}
dirMy ="/home/laurie/Desktop/MEGAsync/iccat-mse"
...
**Figure** Time series of recruits, SSB, biomass, mean F, F apex and Catch
...
**Figure**  Stock recruitment fit with diagnostics for base case.

```{ralb-om-eql,fig.width=8}
...
**Figure**  Equilibrium curves

```{ralb-om-prd,fig.height=4}
...
**Figure** Simulation for biomass production function
...
bdt=maply(1:82,function(i) biodyn:::FLBRP2biodyn(iter(bry,i))@params)
...
```{ralb-om-bench1,fig.width=8}
ggplot(bdt)+
  geom_line(aes(as.numeric(as.numeric(iter)+1929),data))+
  facet_wrap(~quantity,scale="free",ncol=2)+
  xlab("Year")+ylab("MSY Benchmarks")+
  theme_bw(10)
```

**Figure** Time series of benchmarks


```{ralb-om-bench2,fig.width=8}
ggplot(ddply(subset(bdt,!(quantity%in%c("Fapex Age","b0","p"))),
...
**Figure** Time series of MSY benchmarks
...
**Figure** Surplus production, lines are expected values (red is the non-stationary function) and points are the observered values.
...
**Figure** Cross correlation between production and recruitment, showing production is driven by recruitment 5 to 10 years earlier.